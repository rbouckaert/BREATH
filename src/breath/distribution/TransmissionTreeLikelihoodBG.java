...
public class TransmissionTreeLikelihoodBG extends TransmissionTreeLikelihood {
...
//    final public Input<Double> originBranchLengthThresholdInput = new Input<>("originBranchLengthThreshold", "minimal origin branch length (i.e. branch above the root of the tree) for which penalty applies (to prevent very samll branch lengths)", 1e-4);
	...
	public void initAndValidate() {
		...
		// lambda = (Cs*f*Ctr + (1-Cs)*Ctr) ; // CC: Just set lambda = Ctr 
		lambda = Ctr;
...
//System.err.println("#node " + (i+1));
			...
			//logP1 += logh_s(start, end) + logS_s(start, end); // CC: if this is for a sampled case I think it should be logh_s(start,end) + log(q) where q is the sampling probability and h_s is just dgamma( ) (NO SCALING FACTOR)
			logP1 += Math.log(dgamma(start-end, As, Bs)) + Math.log(q);
			...
			//logP1 -= logGetIndivCondition(p0, start, d); // CC: this will have to change to P(observed); darker purple term in eqs (2), (4) of L-B-G notes.
			// d = end of study
			logP1 -= Math.log(Pobserved(start, d));
...
//System.err.println("#node " + (i+1));
				...
				// contribution of not being sampled // CC: not sampled at all or just in these intervals? 
				SegmentIntervalList intervals = segments.get(i);
				if (intervals != null) {
					double start = intervals.birthTime;
					// Double logP1 = logS_s(start, d);  // CC:  we need a change here to Prob(unsamp) , not sure what d means though
					Double logP1 = Math.log(1 - q * pgamma(start - d, As, Bs));
					logP1 -= Math.log(Pobserved(start, d));
...
//    			System.err.println((tree.getRoot().getHeight() - end) + " - " + (tree.getRoot().getHeight() - start) + " = " + tau + " logPBlock=" + logPBlock);    			
	...
	private double Pobserved(double start, double end) {
		return 1 - (1-q * pgamma(start- end, As, Bs)) * Math.pow( Math.exp(logS_tr(start, end)), 1-p0);
	}
    ...
    // CC: I think in the BG model we won't need S_s and logS_s.. 
